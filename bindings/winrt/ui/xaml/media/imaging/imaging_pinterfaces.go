...
// IAsyncOperationOfSvgImageSourceLoadStatus is the WinRT interface Windows.Foundation.IAsyncOperation`1<Windows.UI.Xaml.Media.Imaging.SvgImageSourceLoadStatus>.
// IID: f19df5c2-2b78-53a9-8d38-5ca8dbb5dbc6
// Requires: Windows.Foundation.IAsyncInfo.
...
var IID_IAsyncOperationOfSvgImageSourceLoadStatus = win32.GUID{Data1: 0xf19df5c2, Data2: 0x2b78, Data3: 0x53a9, Data4: [8]byte{0x8d, 0x38, 0x5c, 0xa8, 0xdb, 0xb5, 0xdb, 0xc6}}
...
// Await registers a Completed handler and blocks until IAsyncOperationOfSvgImageSourceLoadStatus reaches
// a terminal state, then returns GetResults() — or, when the status is not
// Completed, an error carrying the status and the IAsyncInfo error code (see
// winrt.AsyncError). Safe on an operation that already completed: WinRT
// invokes a handler assigned after completion immediately. put_Completed
...
// — it never contends with the runtime's callback worker, so a completed
// operation cannot deadlock Await.
func (self *IAsyncOperationOfSvgImageSourceLoadStatus) Await() (SvgImageSourceLoadStatus, error) {
	completion := make(chan foundation.AsyncStatus, 1)
	handler, err := NewAsyncOperationCompletedHandlerOfSvgImageSourceLoadStatus(func(_ *IAsyncOperationOfSvgImageSourceLoadStatus, asyncStatus foundation.AsyncStatus) {
		completion <- asyncStatus
	})
	if err != nil {
		return 0, err

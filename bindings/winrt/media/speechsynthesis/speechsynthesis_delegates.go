...
// AsyncOperationCompletedHandlerOfSpeechSynthesisStream is a Go-implemented handler for the WinRT delegate
// Windows.Foundation.AsyncOperationCompletedHandler`1<Windows.Media.SpeechSynthesis.SpeechSynthesisStream>.
// IID: c972b996-6165-50d4-af60-a8c3df51d092
type AsyncOperationCompletedHandlerOfSpeechSynthesisStream struct {
	delegate *winrt.Delegate
}

// IID_AsyncOperationCompletedHandlerOfSpeechSynthesisStream is the delegate identifier for AsyncOperationCompletedHandlerOfSpeechSynthesisStream.
var IID_AsyncOperationCompletedHandlerOfSpeechSynthesisStream = win32.GUID{Data1: 0xc972b996, Data2: 0x6165, Data3: 0x50d4, Data4: [8]byte{0xaf, 0x60, 0xa8, 0xc3, 0xdf, 0x51, 0xd0, 0x92}}

// NewAsyncOperationCompletedHandlerOfSpeechSynthesisStream wraps fn as a COM-callable Windows.Foundation.AsyncOperationCompletedHandler`1<Windows.Media.SpeechSynthesis.SpeechSynthesisStream>.
// The handler starts with one Go-held reference; Close it once no native
// code can still invoke it.
// Pointer-typed callback arguments are BORROWED references owned by the
// event source for the duration of the callback: do not Release them or
// retain them past its return.
func NewAsyncOperationCompletedHandlerOfSpeechSynthesisStream(fn func(asyncInfo *IAsyncOperationOfSpeechSynthesisStream, asyncStatus foundation.AsyncStatus)) (*AsyncOperationCompletedHandlerOfSpeechSynthesisStream, error) {
	delegate, err := winrt.NewDelegate(IID_AsyncOperationCompletedHandlerOfSpeechSynthesisStream, 2, func(raw []uintptr) uintptr {
		fn((*IAsyncOperationOfSpeechSynthesisStream)(unsafe.Pointer(raw[0])), foundation.AsyncStatus(raw[1]))
		return 0
	})
	if err != nil {
		return nil, err
	}
	return &AsyncOperationCompletedHandlerOfSpeechSynthesisStream{delegate: delegate}, nil
}

// Ptr is the COM object pointer an Add<Event> method registers.
func (h *AsyncOperationCompletedHandlerOfSpeechSynthesisStream) Ptr() uintptr {
...
// still invoke the handler (after the event source removed it or closed);
// the runtime keeps its own references while the handler stays registered.
func (h *AsyncOperationCompletedHandlerOfSpeechSynthesisStream) Close() { h.delegate.Release() }

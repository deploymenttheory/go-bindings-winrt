...
// IAsyncOperationOfForceFeedbackLoadEffectResult is the WinRT interface Windows.Foundation.IAsyncOperation`1<Windows.Gaming.Input.ForceFeedback.ForceFeedbackLoadEffectResult>.
// IID: 21f834fc-e845-5ab9-bf85-9534e2397798
// Requires: Windows.Foundation.IAsyncInfo.
type IAsyncOperationOfForceFeedbackLoadEffectResult struct {
	syswinrt.IInspectable
}

// IID_IAsyncOperationOfForceFeedbackLoadEffectResult is the interface identifier for IAsyncOperationOfForceFeedbackLoadEffectResult.
var IID_IAsyncOperationOfForceFeedbackLoadEffectResult = win32.GUID{Data1: 0x21f834fc, Data2: 0xe845, Data3: 0x5ab9, Data4: [8]byte{0xbf, 0x85, 0x95, 0x34, 0xe2, 0x39, 0x77, 0x98}}

// SetCompleted (propput put_Completed) dispatches through IAsyncOperationOfForceFeedbackLoadEffectResult's vtable slot 6.
// A nil handler passes NULL at the ABI (WinRT accepts it where a handler may be cleared).
func (self *IAsyncOperationOfForceFeedbackLoadEffectResult) SetCompleted(handler *AsyncOperationCompletedHandlerOfForceFeedbackLoadEffectResult) error {
	_handler := uintptr(0)
	if handler != nil {
		_handler = handler.Ptr()
	}
...
// GetResults dispatches through IAsyncOperationOfForceFeedbackLoadEffectResult's vtable slot 8.
func (self *IAsyncOperationOfForceFeedbackLoadEffectResult) GetResults() (ForceFeedbackLoadEffectResult, error) {
	result := new(ForceFeedbackLoadEffectResult)
	r1, _, _ := syscall.SyscallN(self.LpVtbl[8], uintptr(unsafe.Pointer(self)), uintptr(winrt.OutParam(unsafe.Pointer(result))))
	return *result, win32.ErrIfFailed(int32(r1))
}

// Await registers a Completed handler and blocks until IAsyncOperationOfForceFeedbackLoadEffectResult reaches
// a terminal state, then returns GetResults() — or, when the status is not
// Completed, an error carrying the status and the IAsyncInfo error code (see
// winrt.AsyncError). Safe on an operation that already completed: WinRT
// invokes a handler assigned after completion immediately. put_Completed
...
// — it never contends with the runtime's callback worker, so a completed
// operation cannot deadlock Await.
func (self *IAsyncOperationOfForceFeedbackLoadEffectResult) Await() (ForceFeedbackLoadEffectResult, error) {
	completion := make(chan foundation.AsyncStatus, 1)
	handler, err := NewAsyncOperationCompletedHandlerOfForceFeedbackLoadEffectResult(func(_ *IAsyncOperationOfForceFeedbackLoadEffectResult, asyncStatus foundation.AsyncStatus) {
		completion <- asyncStatus
	})
	if err != nil {
		return 0, err

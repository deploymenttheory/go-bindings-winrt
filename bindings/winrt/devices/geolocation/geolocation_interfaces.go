// Code generated by go-bindings-winrt-codegen. DO NOT EDIT.

//go:build windows && amd64

package geolocation

import (
	"math"
	"syscall"
	"unsafe"

	"github.com/deploymenttheory/go-bindings-win32/bindings/runtime/win32"
	syswinrt "github.com/deploymenttheory/go-bindings-win32/bindings/win32/system/winrt"
	"github.com/deploymenttheory/go-bindings-winrt/bindings/runtime/winrt"
	"github.com/deploymenttheory/go-bindings-winrt/bindings/winrt/foundation"
)

// ICivicAddress is the WinRT interface Windows.Devices.Geolocation.ICivicAddress.
// IID: a8567a1a-64f4-4d48-bcea-f6b008eca34c
// Exclusive to the Windows.Devices.Geolocation.CivicAddress runtime class.
type ICivicAddress struct {
	syswinrt.IInspectable
}

// IID_ICivicAddress is the interface identifier for ICivicAddress.
var IID_ICivicAddress = win32.GUID{Data1: 0xa8567a1a, Data2: 0x64f4, Data3: 0x4d48, Data4: [8]byte{0xbc, 0xea, 0xf6, 0xb0, 0x08, 0xec, 0xa3, 0x4c}}

// Country (propget get_Country) dispatches through ICivicAddress's vtable slot 6.
func (self *ICivicAddress) Country() (string, error) {
	result := new(syswinrt.HSTRING)
	r1, _, _ := syscall.SyscallN(self.LpVtbl[6], uintptr(unsafe.Pointer(self)), uintptr(winrt.OutParam(unsafe.Pointer(result))))
	if err := win32.ErrIfFailed(int32(r1)); err != nil {
		return "", err
	}
	return winrt.TakeHString(*result), nil
}

// State (propget get_State) dispatches through ICivicAddress's vtable slot 7.
func (self *ICivicAddress) State() (string, error) {
	result := new(syswinrt.HSTRING)
	r1, _, _ := syscall.SyscallN(self.LpVtbl[7], uintptr(unsafe.Pointer(self)), uintptr(winrt.OutParam(unsafe.Pointer(result))))
	if err := win32.ErrIfFailed(int32(r1)); err != nil {
		return "", err
	}
	return winrt.TakeHString(*result), nil
}

// City (propget get_City) dispatches through ICivicAddress's vtable slot 8.
func (self *ICivicAddress) City() (string, error) {
	result := new(syswinrt.HSTRING)
	r1, _, _ := syscall.SyscallN(self.LpVtbl[8], uintptr(unsafe.Pointer(self)), uintptr(winrt.OutParam(unsafe.Pointer(result))))
	if err := win32.ErrIfFailed(int32(r1)); err != nil {
		return "", err
	}
	return winrt.TakeHString(*result), nil
}

// PostalCode (propget get_PostalCode) dispatches through ICivicAddress's vtable slot 9.
func (self *ICivicAddress) PostalCode() (string, error) {
	result := new(syswinrt.HSTRING)
	r1, _, _ := syscall.SyscallN(self.LpVtbl[9], uintptr(unsafe.Pointer(self)), uintptr(winrt.OutParam(unsafe.Pointer(result))))
	if err := win32.ErrIfFailed(int32(r1)); err != nil {
		return "", err
	}
	return winrt.TakeHString(*result), nil
}

// Timestamp (propget get_Timestamp) dispatches through ICivicAddress's vtable slot 10.
func (self *ICivicAddress) Timestamp() (foundation.DateTime, error) {
	result := new(foundation.DateTime)
	r1, _, _ := syscall.SyscallN(self.LpVtbl[10], uintptr(unsafe.Pointer(self)), uintptr(winrt.OutParam(unsafe.Pointer(result))))
	return *result, win32.ErrIfFailed(int32(r1))
}

// IGeoboundingBox is the WinRT interface Windows.Devices.Geolocation.IGeoboundingBox.
// IID: 0896c80b-274f-43da-9a06-cbfcdaeb4ec2
// Exclusive to the Windows.Devices.Geolocation.GeoboundingBox runtime class.
// Requires: Windows.Devices.Geolocation.IGeoshape.
type IGeoboundingBox struct {
	syswinrt.IInspectable
}

// IID_IGeoboundingBox is the interface identifier for IGeoboundingBox.
var IID_IGeoboundingBox = win32.GUID{Data1: 0x0896c80b, Data2: 0x274f, Data3: 0x43da, Data4: [8]byte{0x9a, 0x06, 0xcb, 0xfc, 0xda, 0xeb, 0x4e, 0xc2}}

// NorthwestCorner (propget get_NorthwestCorner) dispatches through IGeoboundingBox's vtable slot 6.
func (self *IGeoboundingBox) NorthwestCorner() (BasicGeoposition, error) {
	result := new(BasicGeoposition)
	r1, _, _ := syscall.SyscallN(self.LpVtbl[6], uintptr(unsafe.Pointer(self)), uintptr(winrt.OutParam(unsafe.Pointer(result))))
	return *result, win32.ErrIfFailed(int32(r1))
}

// SoutheastCorner (propget get_SoutheastCorner) dispatches through IGeoboundingBox's vtable slot 7.
func (self *IGeoboundingBox) SoutheastCorner() (BasicGeoposition, error) {
	result := new(BasicGeoposition)
	r1, _, _ := syscall.SyscallN(self.LpVtbl[7], uintptr(unsafe.Pointer(self)), uintptr(winrt.OutParam(unsafe.Pointer(result))))
	return *result, win32.ErrIfFailed(int32(r1))
}

// Center (propget get_Center) dispatches through IGeoboundingBox's vtable slot 8.
func (self *IGeoboundingBox) Center() (BasicGeoposition, error) {
	result := new(BasicGeoposition)
	r1, _, _ := syscall.SyscallN(self.LpVtbl[8], uintptr(unsafe.Pointer(self)), uintptr(winrt.OutParam(unsafe.Pointer(result))))
	return *result, win32.ErrIfFailed(int32(r1))
}

// MinAltitude (propget get_MinAltitude) dispatches through IGeoboundingBox's vtable slot 9.
func (self *IGeoboundingBox) MinAltitude() (float64, error) {
	result := new(float64)
	r1, _, _ := syscall.SyscallN(self.LpVtbl[9], uintptr(unsafe.Pointer(self)), uintptr(winrt.OutParam(unsafe.Pointer(result))))
	return *result, win32.ErrIfFailed(int32(r1))
}

// MaxAltitude (propget get_MaxAltitude) dispatches through IGeoboundingBox's vtable slot 10.
func (self *IGeoboundingBox) MaxAltitude() (float64, error) {
	result := new(float64)
	r1, _, _ := syscall.SyscallN(self.LpVtbl[10], uintptr(unsafe.Pointer(self)), uintptr(winrt.OutParam(unsafe.Pointer(result))))
	return *result, win32.ErrIfFailed(int32(r1))
}

// IGeoboundingBoxFactory is the WinRT interface Windows.Devices.Geolocation.IGeoboundingBoxFactory.
// IID: 4dfba589-0411-4abc-b3b5-5bbccb57d98c
// Exclusive to the Windows.Devices.Geolocation.GeoboundingBox runtime class.
type IGeoboundingBoxFactory struct {
	syswinrt.IInspectable
}

// IID_IGeoboundingBoxFactory is the interface identifier for IGeoboundingBoxFactory.
var IID_IGeoboundingBoxFactory = win32.GUID{Data1: 0x4dfba589, Data2: 0x0411, Data3: 0x4abc, Data4: [8]byte{0xb3, 0xb5, 0x5b, 0xbc, 0xcb, 0x57, 0xd9, 0x8c}}

// Create dispatches through IGeoboundingBoxFactory's vtable slot 6.
func (self *IGeoboundingBoxFactory) Create(northwestCorner BasicGeoposition, southeastCorner BasicGeoposition) (*IGeoboundingBox, error) {
	result := new(*IGeoboundingBox)
	r1, _, _ := syscall.SyscallN(self.LpVtbl[6], uintptr(unsafe.Pointer(self)), uintptr(winrt.OutParam(unsafe.Pointer(&northwestCorner))), uintptr(winrt.OutParam(unsafe.Pointer(&southeastCorner))), uintptr(winrt.OutParam(unsafe.Pointer(result))))
	return *result, win32.ErrIfFailed(int32(r1))
}

// CreateWithAltitudeReference dispatches through IGeoboundingBoxFactory's vtable slot 7.
func (self *IGeoboundingBoxFactory) CreateWithAltitudeReference(northwestCorner BasicGeoposition, southeastCorner BasicGeoposition, altitudeReferenceSystem AltitudeReferenceSystem) (*IGeoboundingBox, error) {
	result := new(*IGeoboundingBox)
	r1, _, _ := syscall.SyscallN(self.LpVtbl[7], uintptr(unsafe.Pointer(self)), uintptr(winrt.OutParam(unsafe.Pointer(&northwestCorner))), uintptr(winrt.OutParam(unsafe.Pointer(&southeastCorner))), uintptr(altitudeReferenceSystem), uintptr(winrt.OutParam(unsafe.Pointer(result))))
	return *result, win32.ErrIfFailed(int32(r1))
}

// CreateWithAltitudeReferenceAndSpatialReference dispatches through IGeoboundingBoxFactory's vtable slot 8.
func (self *IGeoboundingBoxFactory) CreateWithAltitudeReferenceAndSpatialReference(northwestCorner BasicGeoposition, southeastCorner BasicGeoposition, altitudeReferenceSystem AltitudeReferenceSystem, spatialReferenceId uint32) (*IGeoboundingBox, error) {
	result := new(*IGeoboundingBox)
	r1, _, _ := syscall.SyscallN(self.LpVtbl[8], uintptr(unsafe.Pointer(self)), uintptr(winrt.OutParam(unsafe.Pointer(&northwestCorner))), uintptr(winrt.OutParam(unsafe.Pointer(&southeastCorner))), uintptr(altitudeReferenceSystem), uintptr(spatialReferenceId), uintptr(winrt.OutParam(unsafe.Pointer(result))))
	return *result, win32.ErrIfFailed(int32(r1))
}

// IGeoboundingBoxStatics is the WinRT interface Windows.Devices.Geolocation.IGeoboundingBoxStatics.
// IID: 67b80708-e61a-4cd0-841b-93233792b5ca
// Exclusive to the Windows.Devices.Geolocation.GeoboundingBox runtime class.
type IGeoboundingBoxStatics struct {
	syswinrt.IInspectable
}

// IID_IGeoboundingBoxStatics is the interface identifier for IGeoboundingBoxStatics.
var IID_IGeoboundingBoxStatics = win32.GUID{Data1: 0x67b80708, Data2: 0xe61a, Data3: 0x4cd0, Data4: [8]byte{0x84, 0x1b, 0x93, 0x23, 0x37, 0x92, 0xb5, 0xca}}

// TryCompute dispatches through IGeoboundingBoxStatics's vtable slot 6.
func (self *IGeoboundingBoxStatics) TryCompute(positions *IIterableOfBasicGeoposition) (*IGeoboundingBox, error) {
	result := new(*IGeoboundingBox)
	r1, _, _ := syscall.SyscallN(self.LpVtbl[6], uintptr(unsafe.Pointer(self)), uintptr(unsafe.Pointer(positions)), uintptr(winrt.OutParam(unsafe.Pointer(result))))
	return *result, win32.ErrIfFailed(int32(r1))
}

// TryComputeWithAltitudeReference dispatches through IGeoboundingBoxStatics's vtable slot 7.
func (self *IGeoboundingBoxStatics) TryComputeWithAltitudeReference(positions *IIterableOfBasicGeoposition, altitudeRefSystem AltitudeReferenceSystem) (*IGeoboundingBox, error) {
	result := new(*IGeoboundingBox)
	r1, _, _ := syscall.SyscallN(self.LpVtbl[7], uintptr(unsafe.Pointer(self)), uintptr(unsafe.Pointer(positions)), uintptr(altitudeRefSystem), uintptr(winrt.OutParam(unsafe.Pointer(result))))
	return *result, win32.ErrIfFailed(int32(r1))
}

// TryComputeWithAltitudeReferenceAndSpatialReference dispatches through IGeoboundingBoxStatics's vtable slot 8.
func (self *IGeoboundingBoxStatics) TryComputeWithAltitudeReferenceAndSpatialReference(positions *IIterableOfBasicGeoposition, altitudeRefSystem AltitudeReferenceSystem, spatialReferenceId uint32) (*IGeoboundingBox, error) {
	result := new(*IGeoboundingBox)
	r1, _, _ := syscall.SyscallN(self.LpVtbl[8], uintptr(unsafe.Pointer(self)), uintptr(unsafe.Pointer(positions)), uintptr(altitudeRefSystem), uintptr(spatialReferenceId), uintptr(winrt.OutParam(unsafe.Pointer(result))))
	return *result, win32.ErrIfFailed(int32(r1))
}

// IGeocircle is the WinRT interface Windows.Devices.Geolocation.IGeocircle.
// IID: 39e45843-a7f9-4e63-92a7-ba0c28d124b1
// Exclusive to the Windows.Devices.Geolocation.Geocircle runtime class.
// Requires: Windows.Devices.Geolocation.IGeoshape.
type IGeocircle struct {
	syswinrt.IInspectable
}

// IID_IGeocircle is the interface identifier for IGeocircle.
var IID_IGeocircle = win32.GUID{Data1: 0x39e45843, Data2: 0xa7f9, Data3: 0x4e63, Data4: [8]byte{0x92, 0xa7, 0xba, 0x0c, 0x28, 0xd1, 0x24, 0xb1}}

// Center (propget get_Center) dispatches through IGeocircle's vtable slot 6.
func (self *IGeocircle) Center() (BasicGeoposition, error) {
	result := new(BasicGeoposition)
	r1, _, _ := syscall.SyscallN(self.LpVtbl[6], uintptr(unsafe.Pointer(self)), uintptr(winrt.OutParam(unsafe.Pointer(result))))
	return *result, win32.ErrIfFailed(int32(r1))
}

// Radius (propget get_Radius) dispatches through IGeocircle's vtable slot 7.
func (self *IGeocircle) Radius() (float64, error) {
	result := new(float64)
	r1, _, _ := syscall.SyscallN(self.LpVtbl[7], uintptr(unsafe.Pointer(self)), uintptr(winrt.OutParam(unsafe.Pointer(result))))
	return *result, win32.ErrIfFailed(int32(r1))
}

// IGeocircleFactory is the WinRT interface Windows.Devices.Geolocation.IGeocircleFactory.
// IID: afd6531f-72b1-4f7d-87cc-4ed4c9849c05
// Exclusive to the Windows.Devices.Geolocation.Geocircle runtime class.
type IGeocircleFactory struct {
	syswinrt.IInspectable
}

// IID_IGeocircleFactory is the interface identifier for IGeocircleFactory.
var IID_IGeocircleFactory = win32.GUID{Data1: 0xafd6531f, Data2: 0x72b1, Data3: 0x4f7d, Data4: [8]byte{0x87, 0xcc, 0x4e, 0xd4, 0xc9, 0x84, 0x9c, 0x05}}

// Create dispatches through IGeocircleFactory's vtable slot 6.
func (self *IGeocircleFactory) Create(position BasicGeoposition, radius float64) (*IGeocircle, error) {
	_radius := uintptr(math.Float64bits(radius))
	result := new(*IGeocircle)
	r1, _, _ := syscall.SyscallN(self.LpVtbl[6], uintptr(unsafe.Pointer(self)), uintptr(winrt.OutParam(unsafe.Pointer(&position))), _radius, uintptr(winrt.OutParam(unsafe.Pointer(result))))
	return *result, win32.ErrIfFailed(int32(r1))
}

// CreateWithAltitudeReferenceSystem dispatches through IGeocircleFactory's vtable slot 7.
func (self *IGeocircleFactory) CreateWithAltitudeReferenceSystem(position BasicGeoposition, radius float64, altitudeReferenceSystem AltitudeReferenceSystem) (*IGeocircle, error) {
	_radius := uintptr(math.Float64bits(radius))
	result := new(*IGeocircle)
	r1, _, _ := syscall.SyscallN(self.LpVtbl[7], uintptr(unsafe.Pointer(self)), uintptr(winrt.OutParam(unsafe.Pointer(&position))), _radius, uintptr(altitudeReferenceSystem), uintptr(winrt.OutParam(unsafe.Pointer(result))))
	return *result, win32.ErrIfFailed(int32(r1))
}

// CreateWithAltitudeReferenceSystemAndSpatialReferenceId dispatches through IGeocircleFactory's vtable slot 8.
func (self *IGeocircleFactory) CreateWithAltitudeReferenceSystemAndSpatialReferenceId(position BasicGeoposition, radius float64, altitudeReferenceSystem AltitudeReferenceSystem, spatialReferenceId uint32) (*IGeocircle, error) {
	_radius := uintptr(math.Float64bits(radius))
	result := new(*IGeocircle)
	r1, _, _ := syscall.SyscallN(self.LpVtbl[8], uintptr(unsafe.Pointer(self)), uintptr(winrt.OutParam(unsafe.Pointer(&position))), _radius, uintptr(altitudeReferenceSystem), uintptr(spatialReferenceId), uintptr(winrt.OutParam(unsafe.Pointer(result))))
	return *result, win32.ErrIfFailed(int32(r1))
}

// IGeocoordinate is the WinRT interface Windows.Devices.Geolocation.IGeocoordinate.
// IID: ee21a3aa-976a-4c70-803d-083ea55bcbc4
// Exclusive to the Windows.Devices.Geolocation.Geocoordinate runtime class.
type IGeocoordinate struct {
	syswinrt.IInspectable
}

// IID_IGeocoordinate is the interface identifier for IGeocoordinate.
var IID_IGeocoordinate = win32.GUID{Data1: 0xee21a3aa, Data2: 0x976a, Data3: 0x4c70, Data4: [8]byte{0x80, 0x3d, 0x08, 0x3e, 0xa5, 0x5b, 0xcb, 0xc4}}

// Latitude (propget get_Latitude) dispatches through IGeocoordinate's vtable slot 6.
func (self *IGeocoordinate) Latitude() (float64, error) {
	result := new(float64)
	r1, _, _ := syscall.SyscallN(self.LpVtbl[6], uintptr(unsafe.Pointer(self)), uintptr(winrt.OutParam(unsafe.Pointer(result))))
	return *result, win32.ErrIfFailed(int32(r1))
}

// Longitude (propget get_Longitude) dispatches through IGeocoordinate's vtable slot 7.
func (self *IGeocoordinate) Longitude() (float64, error) {
	result := new(float64)
	r1, _, _ := syscall.SyscallN(self.LpVtbl[7], uintptr(unsafe.Pointer(self)), uintptr(winrt.OutParam(unsafe.Pointer(result))))
	return *result, win32.ErrIfFailed(int32(r1))
}

// Altitude (propget get_Altitude) dispatches through IGeocoordinate's vtable slot 8.
func (self *IGeocoordinate) Altitude() (*IReferenceOfDouble, error) {
	result := new(*IReferenceOfDouble)
	r1, _, _ := syscall.SyscallN(self.LpVtbl[8], uintptr(unsafe.Pointer(self)), uintptr(winrt.OutParam(unsafe.Pointer(result))))
	return *result, win32.ErrIfFailed(int32(r1))
}

// Accuracy (propget get_Accuracy) dispatches through IGeocoordinate's vtable slot 9.
func (self *IGeocoordinate) Accuracy() (float64, error) {
	result := new(float64)
	r1, _, _ := syscall.SyscallN(self.LpVtbl[9], uintptr(unsafe.Pointer(self)), uintptr(winrt.OutParam(unsafe.Pointer(result))))
	return *result, win32.ErrIfFailed(int32(r1))
}

// AltitudeAccuracy (propget get_AltitudeAccuracy) dispatches through IGeocoordinate's vtable slot 10.
func (self *IGeocoordinate) AltitudeAccuracy() (*IReferenceOfDouble, error) {
	result := new(*IReferenceOfDouble)
	r1, _, _ := syscall.SyscallN(self.LpVtbl[10], uintptr(unsafe.Pointer(self)), uintptr(winrt.OutParam(unsafe.Pointer(result))))
	return *result, win32.ErrIfFailed(int32(r1))
}

// Heading (propget get_Heading) dispatches through IGeocoordinate's vtable slot 11.
func (self *IGeocoordinate) Heading() (*IReferenceOfDouble, error) {
	result := new(*IReferenceOfDouble)
	r1, _, _ := syscall.SyscallN(self.LpVtbl[11], uintptr(unsafe.Pointer(self)), uintptr(winrt.OutParam(unsafe.Pointer(result))))
	return *result, win32.ErrIfFailed(int32(r1))
}

// Speed (propget get_Speed) dispatches through IGeocoordinate's vtable slot 12.
func (self *IGeocoordinate) Speed() (*IReferenceOfDouble, error) {
	result := new(*IReferenceOfDouble)
	r1, _, _ := syscall.SyscallN(self.LpVtbl[12], uintptr(unsafe.Pointer(self)), uintptr(winrt.OutParam(unsafe.Pointer(result))))
	return *result, win32.ErrIfFailed(int32(r1))
}

// Timestamp (propget get_Timestamp) dispatches through IGeocoordinate's vtable slot 13.
func (self *IGeocoordinate) Timestamp() (foundation.DateTime, error) {
	result := new(foundation.DateTime)
	r1, _, _ := syscall.SyscallN(self.LpVtbl[13], uintptr(unsafe.Pointer(self)), uintptr(winrt.OutParam(unsafe.Pointer(result))))
	return *result, win32.ErrIfFailed(int32(r1))
}

// IGeocoordinateSatelliteData is the WinRT interface Windows.Devices.Geolocation.IGeocoordinateSatelliteData.
// IID: c32a74d9-2608-474c-912c-06dd490f4af7
// Exclusive to the Windows.Devices.Geolocation.GeocoordinateSatelliteData runtime class.
type IGeocoordinateSatelliteData struct {
	syswinrt.IInspectable
}

// IID_IGeocoordinateSatelliteData is the interface identifier for IGeocoordinateSatelliteData.
var IID_IGeocoordinateSatelliteData = win32.GUID{Data1: 0xc32a74d9, Data2: 0x2608, Data3: 0x474c, Data4: [8]byte{0x91, 0x2c, 0x06, 0xdd, 0x49, 0x0f, 0x4a, 0xf7}}

// PositionDilutionOfPrecision (propget get_PositionDilutionOfPrecision) dispatches through IGeocoordinateSatelliteData's vtable slot 6.
func (self *IGeocoordinateSatelliteData) PositionDilutionOfPrecision() (*IReferenceOfDouble, error) {
	result := new(*IReferenceOfDouble)
	r1, _, _ := syscall.SyscallN(self.LpVtbl[6], uintptr(unsafe.Pointer(self)), uintptr(winrt.OutParam(unsafe.Pointer(result))))
	return *result, win32.ErrIfFailed(int32(r1))
}

// HorizontalDilutionOfPrecision (propget get_HorizontalDilutionOfPrecision) dispatches through IGeocoordinateSatelliteData's vtable slot 7.
func (self *IGeocoordinateSatelliteData) HorizontalDilutionOfPrecision() (*IReferenceOfDouble, error) {
	result := new(*IReferenceOfDouble)
	r1, _, _ := syscall.SyscallN(self.LpVtbl[7], uintptr(unsafe.Pointer(self)), uintptr(winrt.OutParam(unsafe.Pointer(result))))
	return *result, win32.ErrIfFailed(int32(r1))
}

// VerticalDilutionOfPrecision (propget get_VerticalDilutionOfPrecision) dispatches through IGeocoordinateSatelliteData's vtable slot 8.
func (self *IGeocoordinateSatelliteData) VerticalDilutionOfPrecision() (*IReferenceOfDouble, error) {
	result := new(*IReferenceOfDouble)
	r1, _, _ := syscall.SyscallN(self.LpVtbl[8], uintptr(unsafe.Pointer(self)), uintptr(winrt.OutParam(unsafe.Pointer(result))))
	return *result, win32.ErrIfFailed(int32(r1))
}

// IGeocoordinateSatelliteData2 is the WinRT interface Windows.Devices.Geolocation.IGeocoordinateSatelliteData2.
// IID: 761c8cfd-a19d-5a51-80f5-71676115483e
// Exclusive to the Windows.Devices.Geolocation.GeocoordinateSatelliteData runtime class.
type IGeocoordinateSatelliteData2 struct {
	syswinrt.IInspectable
}

// IID_IGeocoordinateSatelliteData2 is the interface identifier for IGeocoordinateSatelliteData2.
var IID_IGeocoordinateSatelliteData2 = win32.GUID{Data1: 0x761c8cfd, Data2: 0xa19d, Data3: 0x5a51, Data4: [8]byte{0x80, 0xf5, 0x71, 0x67, 0x61, 0x15, 0x48, 0x3e}}

// GeometricDilutionOfPrecision (propget get_GeometricDilutionOfPrecision) dispatches through IGeocoordinateSatelliteData2's vtable slot 6.
func (self *IGeocoordinateSatelliteData2) GeometricDilutionOfPrecision() (*IReferenceOfDouble, error) {
	result := new(*IReferenceOfDouble)
	r1, _, _ := syscall.SyscallN(self.LpVtbl[6], uintptr(unsafe.Pointer(self)), uintptr(winrt.OutParam(unsafe.Pointer(result))))
	return *result, win32.ErrIfFailed(int32(r1))
}

// TimeDilutionOfPrecision (propget get_TimeDilutionOfPrecision) dispatches through IGeocoordinateSatelliteData2's vtable slot 7.
func (self *IGeocoordinateSatelliteData2) TimeDilutionOfPrecision() (*IReferenceOfDouble, error) {
	result := new(*IReferenceOfDouble)
	r1, _, _ := syscall.SyscallN(self.LpVtbl[7], uintptr(unsafe.Pointer(self)), uintptr(winrt.OutParam(unsafe.Pointer(result))))
	return *result, win32.ErrIfFailed(int32(r1))
}

// IGeocoordinateWithPoint is the WinRT interface Windows.Devices.Geolocation.IGeocoordinateWithPoint.
// IID: feea0525-d22c-4d46-b527-0b96066fc7db
// Exclusive to the Windows.Devices.Geolocation.Geocoordinate runtime class.
type IGeocoordinateWithPoint struct {
	syswinrt.IInspectable
}

// IID_IGeocoordinateWithPoint is the interface identifier for IGeocoordinateWithPoint.
var IID_IGeocoordinateWithPoint = win32.GUID{Data1: 0xfeea0525, Data2: 0xd22c, Data3: 0x4d46, Data4: [8]byte{0xb5, 0x27, 0x0b, 0x96, 0x06, 0x6f, 0xc7, 0xdb}}

// Point (propget get_Point) dispatches through IGeocoordinateWithPoint's vtable slot 6.
func (self *IGeocoordinateWithPoint) Point() (*IGeopoint, error) {
	result := new(*IGeopoint)
	r1, _, _ := syscall.SyscallN(self.LpVtbl[6], uintptr(unsafe.Pointer(self)), uintptr(winrt.OutParam(unsafe.Pointer(result))))
	return *result, win32.ErrIfFailed(int32(r1))
}

// IGeocoordinateWithPositionData is the WinRT interface Windows.Devices.Geolocation.IGeocoordinateWithPositionData.
// IID: 95e634be-dbd6-40ac-b8f2-a65c0340d9a6
// Exclusive to the Windows.Devices.Geolocation.Geocoordinate runtime class.
// Requires: Windows.Devices.Geolocation.IGeocoordinate.
type IGeocoordinateWithPositionData struct {
	syswinrt.IInspectable
}

// IID_IGeocoordinateWithPositionData is the interface identifier for IGeocoordinateWithPositionData.
var IID_IGeocoordinateWithPositionData = win32.GUID{Data1: 0x95e634be, Data2: 0xdbd6, Data3: 0x40ac, Data4: [8]byte{0xb8, 0xf2, 0xa6, 0x5c, 0x03, 0x40, 0xd9, 0xa6}}

// PositionSource (propget get_PositionSource) dispatches through IGeocoordinateWithPositionData's vtable slot 6.
func (self *IGeocoordinateWithPositionData) PositionSource() (PositionSource, error) {
	result := new(PositionSource)
	r1, _, _ := syscall.SyscallN(self.LpVtbl[6], uintptr(unsafe.Pointer(self)), uintptr(winrt.OutParam(unsafe.Pointer(result))))
	return *result, win32.ErrIfFailed(int32(r1))
}

// SatelliteData (propget get_SatelliteData) dispatches through IGeocoordinateWithPositionData's vtable slot 7.
func (self *IGeocoordinateWithPositionData) SatelliteData() (*IGeocoordinateSatelliteData, error) {
	result := new(*IGeocoordinateSatelliteData)
	r1, _, _ := syscall.SyscallN(self.LpVtbl[7], uintptr(unsafe.Pointer(self)), uintptr(winrt.OutParam(unsafe.Pointer(result))))
	return *result, win32.ErrIfFailed(int32(r1))
}

// IGeocoordinateWithPositionSourceTimestamp is the WinRT interface Windows.Devices.Geolocation.IGeocoordinateWithPositionSourceTimestamp.
// IID: 8543fc02-c9f1-4610-afe0-8bc3a6a87036
// Exclusive to the Windows.Devices.Geolocation.Geocoordinate runtime class.
type IGeocoordinateWithPositionSourceTimestamp struct {
	syswinrt.IInspectable
}

// IID_IGeocoordinateWithPositionSourceTimestamp is the interface identifier for IGeocoordinateWithPositionSourceTimestamp.
var IID_IGeocoordinateWithPositionSourceTimestamp = win32.GUID{Data1: 0x8543fc02, Data2: 0xc9f1, Data3: 0x4610, Data4: [8]byte{0xaf, 0xe0, 0x8b, 0xc3, 0xa6, 0xa8, 0x70, 0x36}}

// PositionSourceTimestamp (propget get_PositionSourceTimestamp) dispatches through IGeocoordinateWithPositionSourceTimestamp's vtable slot 6.
func (self *IGeocoordinateWithPositionSourceTimestamp) PositionSourceTimestamp() (*IReferenceOfDateTime, error) {
	result := new(*IReferenceOfDateTime)
	r1, _, _ := syscall.SyscallN(self.LpVtbl[6], uintptr(unsafe.Pointer(self)), uintptr(winrt.OutParam(unsafe.Pointer(result))))
	return *result, win32.ErrIfFailed(int32(r1))
}

// IGeocoordinateWithRemoteSource is the WinRT interface Windows.Devices.Geolocation.IGeocoordinateWithRemoteSource.
// IID: 397cebd7-ee38-5f3b-8900-c4a7bc9cf953
// Exclusive to the Windows.Devices.Geolocation.Geocoordinate runtime class.
type IGeocoordinateWithRemoteSource struct {
	syswinrt.IInspectable
}

// IID_IGeocoordinateWithRemoteSource is the interface identifier for IGeocoordinateWithRemoteSource.
var IID_IGeocoordinateWithRemoteSource = win32.GUID{Data1: 0x397cebd7, Data2: 0xee38, Data3: 0x5f3b, Data4: [8]byte{0x89, 0x00, 0xc4, 0xa7, 0xbc, 0x9c, 0xf9, 0x53}}

// IsRemoteSource (propget get_IsRemoteSource) dispatches through IGeocoordinateWithRemoteSource's vtable slot 6.
func (self *IGeocoordinateWithRemoteSource) IsRemoteSource() (bool, error) {
	result := new(byte)
	r1, _, _ := syscall.SyscallN(self.LpVtbl[6], uintptr(unsafe.Pointer(self)), uintptr(winrt.OutParam(unsafe.Pointer(result))))
	return *result != 0, win32.ErrIfFailed(int32(r1))
}

// IGeolocator is the WinRT interface Windows.Devices.Geolocation.IGeolocator.
// IID: a9c3bf62-4524-4989-8aa9-de019d2e551f
// Exclusive to the Windows.Devices.Geolocation.Geolocator runtime class.
type IGeolocator struct {
	syswinrt.IInspectable
}

// IID_IGeolocator is the interface identifier for IGeolocator.
var IID_IGeolocator = win32.GUID{Data1: 0xa9c3bf62, Data2: 0x4524, Data3: 0x4989, Data4: [8]byte{0x8a, 0xa9, 0xde, 0x01, 0x9d, 0x2e, 0x55, 0x1f}}

// DesiredAccuracy (propget get_DesiredAccuracy) dispatches through IGeolocator's vtable slot 6.
func (self *IGeolocator) DesiredAccuracy() (PositionAccuracy, error) {
	result := new(PositionAccuracy)
	r1, _, _ := syscall.SyscallN(self.LpVtbl[6], uintptr(unsafe.Pointer(self)), uintptr(winrt.OutParam(unsafe.Pointer(result))))
	return *result, win32.ErrIfFailed(int32(r1))
}

// SetDesiredAccuracy (propput put_DesiredAccuracy) dispatches through IGeolocator's vtable slot 7.
func (self *IGeolocator) SetDesiredAccuracy(value PositionAccuracy) error {
	r1, _, _ := syscall.SyscallN(self.LpVtbl[7], uintptr(unsafe.Pointer(self)), uintptr(value))
	return win32.ErrIfFailed(int32(r1))
}

// MovementThreshold (propget get_MovementThreshold) dispatches through IGeolocator's vtable slot 8.
func (self *IGeolocator) MovementThreshold() (float64, error) {
	result := new(float64)
	r1, _, _ := syscall.SyscallN(self.LpVtbl[8], uintptr(unsafe.Pointer(self)), uintptr(winrt.OutParam(unsafe.Pointer(result))))
	return *result, win32.ErrIfFailed(int32(r1))
}

// SetMovementThreshold (propput put_MovementThreshold) dispatches through IGeolocator's vtable slot 9.
func (self *IGeolocator) SetMovementThreshold(value float64) error {
	_value := uintptr(math.Float64bits(value))
	r1, _, _ := syscall.SyscallN(self.LpVtbl[9], uintptr(unsafe.Pointer(self)), _value)
	return win32.ErrIfFailed(int32(r1))
}

// ReportInterval (propget get_ReportInterval) dispatches through IGeolocator's vtable slot 10.
func (self *IGeolocator) ReportInterval() (uint32, error) {
	result := new(uint32)
	r1, _, _ := syscall.SyscallN(self.LpVtbl[10], uintptr(unsafe.Pointer(self)), uintptr(winrt.OutParam(unsafe.Pointer(result))))
	return *result, win32.ErrIfFailed(int32(r1))
}

// SetReportInterval (propput put_ReportInterval) dispatches through IGeolocator's vtable slot 11.
func (self *IGeolocator) SetReportInterval(value uint32) error {
	r1, _, _ := syscall.SyscallN(self.LpVtbl[11], uintptr(unsafe.Pointer(self)), uintptr(value))
	return win32.ErrIfFailed(int32(r1))
}

// LocationStatus (propget get_LocationStatus) dispatches through IGeolocator's vtable slot 12.
func (self *IGeolocator) LocationStatus() (PositionStatus, error) {
	result := new(PositionStatus)
	r1, _, _ := syscall.SyscallN(self.LpVtbl[12], uintptr(unsafe.Pointer(self)), uintptr(winrt.OutParam(unsafe.Pointer(result))))
	return *result, win32.ErrIfFailed(int32(r1))
}

// GetGeopositionAsync dispatches through IGeolocator's vtable slot 13.
func (self *IGeolocator) GetGeopositionAsync() (*IAsyncOperationOfGeoposition, error) {
	result := new(*IAsyncOperationOfGeoposition)
	r1, _, _ := syscall.SyscallN(self.LpVtbl[13], uintptr(unsafe.Pointer(self)), uintptr(winrt.OutParam(unsafe.Pointer(result))))
	return *result, win32.ErrIfFailed(int32(r1))
}

// GetGeopositionAsyncWithAgeAndTimeout dispatches through IGeolocator's vtable slot 14.
func (self *IGeolocator) GetGeopositionAsyncWithAgeAndTimeout(maximumAge foundation.TimeSpan, timeout foundation.TimeSpan) (*IAsyncOperationOfGeoposition, error) {
	_maximumAge := *(*uintptr)(unsafe.Pointer(&maximumAge))
	_timeout := *(*uintptr)(unsafe.Pointer(&timeout))
	result := new(*IAsyncOperationOfGeoposition)
	r1, _, _ := syscall.SyscallN(self.LpVtbl[14], uintptr(unsafe.Pointer(self)), _maximumAge, _timeout, uintptr(winrt.OutParam(unsafe.Pointer(result))))
	return *result, win32.ErrIfFailed(int32(r1))
}

// AddPositionChanged (event add add_PositionChanged) dispatches through IGeolocator's vtable slot 15.
// The handler stays registered (and referenced by the runtime) until the
// returned token is passed to RemovePositionChanged.
func (self *IGeolocator) AddPositionChanged(handler *TypedEventHandlerOfGeolocatorAndPositionChangedEventArgs) (syswinrt.EventRegistrationToken, error) {
	result := new(syswinrt.EventRegistrationToken)
	r1, _, _ := syscall.SyscallN(self.LpVtbl[15], uintptr(unsafe.Pointer(self)), handler.Ptr(), uintptr(winrt.OutParam(unsafe.Pointer(result))))
	return *result, win32.ErrIfFailed(int32(r1))
}

// RemovePositionChanged (event remove remove_PositionChanged) dispatches through IGeolocator's vtable slot 16,
// unregistering the PositionChanged handler the token was returned for.
func (self *IGeolocator) RemovePositionChanged(token syswinrt.EventRegistrationToken) error {
	r1, _, _ := syscall.SyscallN(self.LpVtbl[16], uintptr(unsafe.Pointer(self)), uintptr(token.Value))
	return win32.ErrIfFailed(int32(r1))
}

// AddStatusChanged (event add add_StatusChanged) dispatches through IGeolocator's vtable slot 17.
// The handler stays registered (and referenced by the runtime) until the
// returned token is passed to RemoveStatusChanged.
func (self *IGeolocator) AddStatusChanged(handler *TypedEventHandlerOfGeolocatorAndStatusChangedEventArgs) (syswinrt.EventRegistrationToken, error) {
	result := new(syswinrt.EventRegistrationToken)
	r1, _, _ := syscall.SyscallN(self.LpVtbl[17], uintptr(unsafe.Pointer(self)), handler.Ptr(), uintptr(winrt.OutParam(unsafe.Pointer(result))))
	return *result, win32.ErrIfFailed(int32(r1))
}

// RemoveStatusChanged (event remove remove_StatusChanged) dispatches through IGeolocator's vtable slot 18,
// unregistering the StatusChanged handler the token was returned for.
func (self *IGeolocator) RemoveStatusChanged(token syswinrt.EventRegistrationToken) error {
	r1, _, _ := syscall.SyscallN(self.LpVtbl[18], uintptr(unsafe.Pointer(self)), uintptr(token.Value))
	return win32.ErrIfFailed(int32(r1))
}

// IGeolocator2 is the WinRT interface Windows.Devices.Geolocation.IGeolocator2.
// IID: d1b42e6d-8891-43b4-ad36-27c6fe9a97b1
// Exclusive to the Windows.Devices.Geolocation.Geolocator runtime class.
type IGeolocator2 struct {
	syswinrt.IInspectable
}

// IID_IGeolocator2 is the interface identifier for IGeolocator2.
var IID_IGeolocator2 = win32.GUID{Data1: 0xd1b42e6d, Data2: 0x8891, Data3: 0x43b4, Data4: [8]byte{0xad, 0x36, 0x27, 0xc6, 0xfe, 0x9a, 0x97, 0xb1}}

// AllowFallbackToConsentlessPositions dispatches through IGeolocator2's vtable slot 6.
func (self *IGeolocator2) AllowFallbackToConsentlessPositions() error {
	r1, _, _ := syscall.SyscallN(self.LpVtbl[6], uintptr(unsafe.Pointer(self)))
	return win32.ErrIfFailed(int32(r1))
}

// IGeolocatorStatics is the WinRT interface Windows.Devices.Geolocation.IGeolocatorStatics.
// IID: 9a8e7571-2df5-4591-9f87-eb5fd894e9b7
// Exclusive to the Windows.Devices.Geolocation.Geolocator runtime class.
type IGeolocatorStatics struct {
	syswinrt.IInspectable
}

// IID_IGeolocatorStatics is the interface identifier for IGeolocatorStatics.
var IID_IGeolocatorStatics = win32.GUID{Data1: 0x9a8e7571, Data2: 0x2df5, Data3: 0x4591, Data4: [8]byte{0x9f, 0x87, 0xeb, 0x5f, 0xd8, 0x94, 0xe9, 0xb7}}

// RequestAccessAsync dispatches through IGeolocatorStatics's vtable slot 6.
func (self *IGeolocatorStatics) RequestAccessAsync() (*IAsyncOperationOfGeolocationAccessStatus, error) {
	result := new(*IAsyncOperationOfGeolocationAccessStatus)
	r1, _, _ := syscall.SyscallN(self.LpVtbl[6], uintptr(unsafe.Pointer(self)), uintptr(winrt.OutParam(unsafe.Pointer(result))))
	return *result, win32.ErrIfFailed(int32(r1))
}

// GetGeopositionHistoryAsync dispatches through IGeolocatorStatics's vtable slot 7.
func (self *IGeolocatorStatics) GetGeopositionHistoryAsync(startTime foundation.DateTime) (*IAsyncOperationOfIVectorViewOfGeoposition, error) {
	_startTime := *(*uintptr)(unsafe.Pointer(&startTime))
	result := new(*IAsyncOperationOfIVectorViewOfGeoposition)
	r1, _, _ := syscall.SyscallN(self.LpVtbl[7], uintptr(unsafe.Pointer(self)), _startTime, uintptr(winrt.OutParam(unsafe.Pointer(result))))
	return *result, win32.ErrIfFailed(int32(r1))
}

// GetGeopositionHistoryWithDurationAsync dispatches through IGeolocatorStatics's vtable slot 8.
func (self *IGeolocatorStatics) GetGeopositionHistoryWithDurationAsync(startTime foundation.DateTime, duration foundation.TimeSpan) (*IAsyncOperationOfIVectorViewOfGeoposition, error) {
	_startTime := *(*uintptr)(unsafe.Pointer(&startTime))
	_duration := *(*uintptr)(unsafe.Pointer(&duration))
	result := new(*IAsyncOperationOfIVectorViewOfGeoposition)
	r1, _, _ := syscall.SyscallN(self.LpVtbl[8], uintptr(unsafe.Pointer(self)), _startTime, _duration, uintptr(winrt.OutParam(unsafe.Pointer(result))))
	return *result, win32.ErrIfFailed(int32(r1))
}

// IGeolocatorStatics2 is the WinRT interface Windows.Devices.Geolocation.IGeolocatorStatics2.
// IID: 993011a2-fa1c-4631-a71d-0dbeb1250d9c
// Exclusive to the Windows.Devices.Geolocation.Geolocator runtime class.
type IGeolocatorStatics2 struct {
	syswinrt.IInspectable
}

// IID_IGeolocatorStatics2 is the interface identifier for IGeolocatorStatics2.
var IID_IGeolocatorStatics2 = win32.GUID{Data1: 0x993011a2, Data2: 0xfa1c, Data3: 0x4631, Data4: [8]byte{0xa7, 0x1d, 0x0d, 0xbe, 0xb1, 0x25, 0x0d, 0x9c}}

// IsDefaultGeopositionRecommended (propget get_IsDefaultGeopositionRecommended) dispatches through IGeolocatorStatics2's vtable slot 6.
func (self *IGeolocatorStatics2) IsDefaultGeopositionRecommended() (bool, error) {
	result := new(byte)
	r1, _, _ := syscall.SyscallN(self.LpVtbl[6], uintptr(unsafe.Pointer(self)), uintptr(winrt.OutParam(unsafe.Pointer(result))))
	return *result != 0, win32.ErrIfFailed(int32(r1))
}

// SetDefaultGeoposition (propput put_DefaultGeoposition) dispatches through IGeolocatorStatics2's vtable slot 7.
func (self *IGeolocatorStatics2) SetDefaultGeoposition(value *IReferenceOfBasicGeoposition) error {
	r1, _, _ := syscall.SyscallN(self.LpVtbl[7], uintptr(unsafe.Pointer(self)), uintptr(unsafe.Pointer(value)))
	return win32.ErrIfFailed(int32(r1))
}

// DefaultGeoposition (propget get_DefaultGeoposition) dispatches through IGeolocatorStatics2's vtable slot 8.
func (self *IGeolocatorStatics2) DefaultGeoposition() (*IReferenceOfBasicGeoposition, error) {
	result := new(*IReferenceOfBasicGeoposition)
	r1, _, _ := syscall.SyscallN(self.LpVtbl[8], uintptr(unsafe.Pointer(self)), uintptr(winrt.OutParam(unsafe.Pointer(result))))
	return *result, win32.ErrIfFailed(int32(r1))
}

// IGeolocatorWithScalarAccuracy is the WinRT interface Windows.Devices.Geolocation.IGeolocatorWithScalarAccuracy.
// IID: 96f5d3c1-b80f-460a-994d-a96c47a51aa4
// Exclusive to the Windows.Devices.Geolocation.Geolocator runtime class.
// Requires: Windows.Devices.Geolocation.IGeolocator.
type IGeolocatorWithScalarAccuracy struct {
	syswinrt.IInspectable
}

// IID_IGeolocatorWithScalarAccuracy is the interface identifier for IGeolocatorWithScalarAccuracy.
var IID_IGeolocatorWithScalarAccuracy = win32.GUID{Data1: 0x96f5d3c1, Data2: 0xb80f, Data3: 0x460a, Data4: [8]byte{0x99, 0x4d, 0xa9, 0x6c, 0x47, 0xa5, 0x1a, 0xa4}}

// DesiredAccuracyInMeters (propget get_DesiredAccuracyInMeters) dispatches through IGeolocatorWithScalarAccuracy's vtable slot 6.
func (self *IGeolocatorWithScalarAccuracy) DesiredAccuracyInMeters() (*IReferenceOfUInt32, error) {
	result := new(*IReferenceOfUInt32)
	r1, _, _ := syscall.SyscallN(self.LpVtbl[6], uintptr(unsafe.Pointer(self)), uintptr(winrt.OutParam(unsafe.Pointer(result))))
	return *result, win32.ErrIfFailed(int32(r1))
}

// SetDesiredAccuracyInMeters (propput put_DesiredAccuracyInMeters) dispatches through IGeolocatorWithScalarAccuracy's vtable slot 7.
func (self *IGeolocatorWithScalarAccuracy) SetDesiredAccuracyInMeters(value *IReferenceOfUInt32) error {
	r1, _, _ := syscall.SyscallN(self.LpVtbl[7], uintptr(unsafe.Pointer(self)), uintptr(unsafe.Pointer(value)))
	return win32.ErrIfFailed(int32(r1))
}

// IGeopath is the WinRT interface Windows.Devices.Geolocation.IGeopath.
// IID: e53fd7b9-2da4-4714-a652-de8593289898
// Exclusive to the Windows.Devices.Geolocation.Geopath runtime class.
// Requires: Windows.Devices.Geolocation.IGeoshape.
type IGeopath struct {
	syswinrt.IInspectable
}

// IID_IGeopath is the interface identifier for IGeopath.
var IID_IGeopath = win32.GUID{Data1: 0xe53fd7b9, Data2: 0x2da4, Data3: 0x4714, Data4: [8]byte{0xa6, 0x52, 0xde, 0x85, 0x93, 0x28, 0x98, 0x98}}

// Positions (propget get_Positions) dispatches through IGeopath's vtable slot 6.
func (self *IGeopath) Positions() (*IVectorViewOfBasicGeoposition, error) {
	result := new(*IVectorViewOfBasicGeoposition)
	r1, _, _ := syscall.SyscallN(self.LpVtbl[6], uintptr(unsafe.Pointer(self)), uintptr(winrt.OutParam(unsafe.Pointer(result))))
	return *result, win32.ErrIfFailed(int32(r1))
}

// IGeopathFactory is the WinRT interface Windows.Devices.Geolocation.IGeopathFactory.
// IID: 27bea9c8-c7e7-4359-9b9b-fca3e05ef593
// Exclusive to the Windows.Devices.Geolocation.Geopath runtime class.
type IGeopathFactory struct {
	syswinrt.IInspectable
}

// IID_IGeopathFactory is the interface identifier for IGeopathFactory.
var IID_IGeopathFactory = win32.GUID{Data1: 0x27bea9c8, Data2: 0xc7e7, Data3: 0x4359, Data4: [8]byte{0x9b, 0x9b, 0xfc, 0xa3, 0xe0, 0x5e, 0xf5, 0x93}}

// Create dispatches through IGeopathFactory's vtable slot 6.
func (self *IGeopathFactory) Create(positions *IIterableOfBasicGeoposition) (*IGeopath, error) {
	result := new(*IGeopath)
	r1, _, _ := syscall.SyscallN(self.LpVtbl[6], uintptr(unsafe.Pointer(self)), uintptr(unsafe.Pointer(positions)), uintptr(winrt.OutParam(unsafe.Pointer(result))))
	return *result, win32.ErrIfFailed(int32(r1))
}

// CreateWithAltitudeReference dispatches through IGeopathFactory's vtable slot 7.
func (self *IGeopathFactory) CreateWithAltitudeReference(positions *IIterableOfBasicGeoposition, altitudeReferenceSystem AltitudeReferenceSystem) (*IGeopath, error) {
	result := new(*IGeopath)
	r1, _, _ := syscall.SyscallN(self.LpVtbl[7], uintptr(unsafe.Pointer(self)), uintptr(unsafe.Pointer(positions)), uintptr(altitudeReferenceSystem), uintptr(winrt.OutParam(unsafe.Pointer(result))))
	return *result, win32.ErrIfFailed(int32(r1))
}

// CreateWithAltitudeReferenceAndSpatialReference dispatches through IGeopathFactory's vtable slot 8.
func (self *IGeopathFactory) CreateWithAltitudeReferenceAndSpatialReference(positions *IIterableOfBasicGeoposition, altitudeReferenceSystem AltitudeReferenceSystem, spatialReferenceId uint32) (*IGeopath, error) {
	result := new(*IGeopath)
	r1, _, _ := syscall.SyscallN(self.LpVtbl[8], uintptr(unsafe.Pointer(self)), uintptr(unsafe.Pointer(positions)), uintptr(altitudeReferenceSystem), uintptr(spatialReferenceId), uintptr(winrt.OutParam(unsafe.Pointer(result))))
	return *result, win32.ErrIfFailed(int32(r1))
}

// IGeopoint is the WinRT interface Windows.Devices.Geolocation.IGeopoint.
// IID: 6bfa00eb-e56e-49bb-9caf-cbaa78a8bcef
// Exclusive to the Windows.Devices.Geolocation.Geopoint runtime class.
// Requires: Windows.Devices.Geolocation.IGeoshape.
type IGeopoint struct {
	syswinrt.IInspectable
}

// IID_IGeopoint is the interface identifier for IGeopoint.
var IID_IGeopoint = win32.GUID{Data1: 0x6bfa00eb, Data2: 0xe56e, Data3: 0x49bb, Data4: [8]byte{0x9c, 0xaf, 0xcb, 0xaa, 0x78, 0xa8, 0xbc, 0xef}}

// Position (propget get_Position) dispatches through IGeopoint's vtable slot 6.
func (self *IGeopoint) Position() (BasicGeoposition, error) {
	result := new(BasicGeoposition)
	r1, _, _ := syscall.SyscallN(self.LpVtbl[6], uintptr(unsafe.Pointer(self)), uintptr(winrt.OutParam(unsafe.Pointer(result))))
	return *result, win32.ErrIfFailed(int32(r1))
}

// IGeopointFactory is the WinRT interface Windows.Devices.Geolocation.IGeopointFactory.
// IID: db6b8d33-76bd-4e30-8af7-a844dc37b7a0
// Exclusive to the Windows.Devices.Geolocation.Geopoint runtime class.
type IGeopointFactory struct {
	syswinrt.IInspectable
}

// IID_IGeopointFactory is the interface identifier for IGeopointFactory.
var IID_IGeopointFactory = win32.GUID{Data1: 0xdb6b8d33, Data2: 0x76bd, Data3: 0x4e30, Data4: [8]byte{0x8a, 0xf7, 0xa8, 0x44, 0xdc, 0x37, 0xb7, 0xa0}}

// Create dispatches through IGeopointFactory's vtable slot 6.
func (self *IGeopointFactory) Create(position BasicGeoposition) (*IGeopoint, error) {
	result := new(*IGeopoint)
	r1, _, _ := syscall.SyscallN(self.LpVtbl[6], uintptr(unsafe.Pointer(self)), uintptr(winrt.OutParam(unsafe.Pointer(&position))), uintptr(winrt.OutParam(unsafe.Pointer(result))))
	return *result, win32.ErrIfFailed(int32(r1))
}

// CreateWithAltitudeReferenceSystem dispatches through IGeopointFactory's vtable slot 7.
func (self *IGeopointFactory) CreateWithAltitudeReferenceSystem(position BasicGeoposition, altitudeReferenceSystem AltitudeReferenceSystem) (*IGeopoint, error) {
	result := new(*IGeopoint)
	r1, _, _ := syscall.SyscallN(self.LpVtbl[7], uintptr(unsafe.Pointer(self)), uintptr(winrt.OutParam(unsafe.Pointer(&position))), uintptr(altitudeReferenceSystem), uintptr(winrt.OutParam(unsafe.Pointer(result))))
	return *result, win32.ErrIfFailed(int32(r1))
}

// CreateWithAltitudeReferenceSystemAndSpatialReferenceId dispatches through IGeopointFactory's vtable slot 8.
func (self *IGeopointFactory) CreateWithAltitudeReferenceSystemAndSpatialReferenceId(position BasicGeoposition, altitudeReferenceSystem AltitudeReferenceSystem, spatialReferenceId uint32) (*IGeopoint, error) {
	result := new(*IGeopoint)
	r1, _, _ := syscall.SyscallN(self.LpVtbl[8], uintptr(unsafe.Pointer(self)), uintptr(winrt.OutParam(unsafe.Pointer(&position))), uintptr(altitudeReferenceSystem), uintptr(spatialReferenceId), uintptr(winrt.OutParam(unsafe.Pointer(result))))
	return *result, win32.ErrIfFailed(int32(r1))
}

// IGeoposition is the WinRT interface Windows.Devices.Geolocation.IGeoposition.
// IID: c18d0454-7d41-4ff7-a957-9dffb4ef7f5b
// Exclusive to the Windows.Devices.Geolocation.Geoposition runtime class.
type IGeoposition struct {
	syswinrt.IInspectable
}

// IID_IGeoposition is the interface identifier for IGeoposition.
var IID_IGeoposition = win32.GUID{Data1: 0xc18d0454, Data2: 0x7d41, Data3: 0x4ff7, Data4: [8]byte{0xa9, 0x57, 0x9d, 0xff, 0xb4, 0xef, 0x7f, 0x5b}}

// Coordinate (propget get_Coordinate) dispatches through IGeoposition's vtable slot 6.
func (self *IGeoposition) Coordinate() (*IGeocoordinate, error) {
	result := new(*IGeocoordinate)
	r1, _, _ := syscall.SyscallN(self.LpVtbl[6], uintptr(unsafe.Pointer(self)), uintptr(winrt.OutParam(unsafe.Pointer(result))))
	return *result, win32.ErrIfFailed(int32(r1))
}

// CivicAddress (propget get_CivicAddress) dispatches through IGeoposition's vtable slot 7.
func (self *IGeoposition) CivicAddress() (*ICivicAddress, error) {
	result := new(*ICivicAddress)
	r1, _, _ := syscall.SyscallN(self.LpVtbl[7], uintptr(unsafe.Pointer(self)), uintptr(winrt.OutParam(unsafe.Pointer(result))))
	return *result, win32.ErrIfFailed(int32(r1))
}

// IGeoposition2 is the WinRT interface Windows.Devices.Geolocation.IGeoposition2.
// IID: 7f62f697-8671-4b0d-86f8-474a8496187c
// Exclusive to the Windows.Devices.Geolocation.Geoposition runtime class.
// Requires: Windows.Devices.Geolocation.IGeoposition.
type IGeoposition2 struct {
	syswinrt.IInspectable
}

// IID_IGeoposition2 is the interface identifier for IGeoposition2.
var IID_IGeoposition2 = win32.GUID{Data1: 0x7f62f697, Data2: 0x8671, Data3: 0x4b0d, Data4: [8]byte{0x86, 0xf8, 0x47, 0x4a, 0x84, 0x96, 0x18, 0x7c}}

// VenueData (propget get_VenueData) dispatches through IGeoposition2's vtable slot 6.
func (self *IGeoposition2) VenueData() (*IVenueData, error) {
	result := new(*IVenueData)
	r1, _, _ := syscall.SyscallN(self.LpVtbl[6], uintptr(unsafe.Pointer(self)), uintptr(winrt.OutParam(unsafe.Pointer(result))))
	return *result, win32.ErrIfFailed(int32(r1))
}

// IGeoshape is the WinRT interface Windows.Devices.Geolocation.IGeoshape.
// IID: c99ca2af-c729-43c1-8fab-d6dec914df7e
type IGeoshape struct {
	syswinrt.IInspectable
}

// IID_IGeoshape is the interface identifier for IGeoshape.
var IID_IGeoshape = win32.GUID{Data1: 0xc99ca2af, Data2: 0xc729, Data3: 0x43c1, Data4: [8]byte{0x8f, 0xab, 0xd6, 0xde, 0xc9, 0x14, 0xdf, 0x7e}}

// GeoshapeType (propget get_GeoshapeType) dispatches through IGeoshape's vtable slot 6.
func (self *IGeoshape) GeoshapeType() (GeoshapeType, error) {
	result := new(GeoshapeType)
	r1, _, _ := syscall.SyscallN(self.LpVtbl[6], uintptr(unsafe.Pointer(self)), uintptr(winrt.OutParam(unsafe.Pointer(result))))
	return *result, win32.ErrIfFailed(int32(r1))
}

// SpatialReferenceId (propget get_SpatialReferenceId) dispatches through IGeoshape's vtable slot 7.
func (self *IGeoshape) SpatialReferenceId() (uint32, error) {
	result := new(uint32)
	r1, _, _ := syscall.SyscallN(self.LpVtbl[7], uintptr(unsafe.Pointer(self)), uintptr(winrt.OutParam(unsafe.Pointer(result))))
	return *result, win32.ErrIfFailed(int32(r1))
}

// AltitudeReferenceSystem (propget get_AltitudeReferenceSystem) dispatches through IGeoshape's vtable slot 8.
func (self *IGeoshape) AltitudeReferenceSystem() (AltitudeReferenceSystem, error) {
	result := new(AltitudeReferenceSystem)
	r1, _, _ := syscall.SyscallN(self.LpVtbl[8], uintptr(unsafe.Pointer(self)), uintptr(winrt.OutParam(unsafe.Pointer(result))))
	return *result, win32.ErrIfFailed(int32(r1))
}

// IGeovisit is the WinRT interface Windows.Devices.Geolocation.IGeovisit.
// IID: b1877a76-9ef6-41ab-a0dd-793ece76e2de
// Exclusive to the Windows.Devices.Geolocation.Geovisit runtime class.
type IGeovisit struct {
	syswinrt.IInspectable
}

// IID_IGeovisit is the interface identifier for IGeovisit.
var IID_IGeovisit = win32.GUID{Data1: 0xb1877a76, Data2: 0x9ef6, Data3: 0x41ab, Data4: [8]byte{0xa0, 0xdd, 0x79, 0x3e, 0xce, 0x76, 0xe2, 0xde}}

// Position (propget get_Position) dispatches through IGeovisit's vtable slot 6.
func (self *IGeovisit) Position() (*IGeoposition, error) {
	result := new(*IGeoposition)
	r1, _, _ := syscall.SyscallN(self.LpVtbl[6], uintptr(unsafe.Pointer(self)), uintptr(winrt.OutParam(unsafe.Pointer(result))))
	return *result, win32.ErrIfFailed(int32(r1))
}

// StateChange (propget get_StateChange) dispatches through IGeovisit's vtable slot 7.
func (self *IGeovisit) StateChange() (VisitStateChange, error) {
	result := new(VisitStateChange)
	r1, _, _ := syscall.SyscallN(self.LpVtbl[7], uintptr(unsafe.Pointer(self)), uintptr(winrt.OutParam(unsafe.Pointer(result))))
	return *result, win32.ErrIfFailed(int32(r1))
}

// Timestamp (propget get_Timestamp) dispatches through IGeovisit's vtable slot 8.
func (self *IGeovisit) Timestamp() (foundation.DateTime, error) {
	result := new(foundation.DateTime)
	r1, _, _ := syscall.SyscallN(self.LpVtbl[8], uintptr(unsafe.Pointer(self)), uintptr(winrt.OutParam(unsafe.Pointer(result))))
	return *result, win32.ErrIfFailed(int32(r1))
}

// IGeovisitMonitor is the WinRT interface Windows.Devices.Geolocation.IGeovisitMonitor.
// IID: 80118aaf-5944-4591-83c1-396647f54f2c
// Exclusive to the Windows.Devices.Geolocation.GeovisitMonitor runtime class.
type IGeovisitMonitor struct {
	syswinrt.IInspectable
}

// IID_IGeovisitMonitor is the interface identifier for IGeovisitMonitor.
var IID_IGeovisitMonitor = win32.GUID{Data1: 0x80118aaf, Data2: 0x5944, Data3: 0x4591, Data4: [8]byte{0x83, 0xc1, 0x39, 0x66, 0x47, 0xf5, 0x4f, 0x2c}}

// MonitoringScope (propget get_MonitoringScope) dispatches through IGeovisitMonitor's vtable slot 6.
func (self *IGeovisitMonitor) MonitoringScope() (VisitMonitoringScope, error) {
	result := new(VisitMonitoringScope)
	r1, _, _ := syscall.SyscallN(self.LpVtbl[6], uintptr(unsafe.Pointer(self)), uintptr(winrt.OutParam(unsafe.Pointer(result))))
	return *result, win32.ErrIfFailed(int32(r1))
}

// Start dispatches through IGeovisitMonitor's vtable slot 7.
func (self *IGeovisitMonitor) Start(value VisitMonitoringScope) error {
	r1, _, _ := syscall.SyscallN(self.LpVtbl[7], uintptr(unsafe.Pointer(self)), uintptr(value))
	return win32.ErrIfFailed(int32(r1))
}

// Stop dispatches through IGeovisitMonitor's vtable slot 8.
func (self *IGeovisitMonitor) Stop() error {
	r1, _, _ := syscall.SyscallN(self.LpVtbl[8], uintptr(unsafe.Pointer(self)))
	return win32.ErrIfFailed(int32(r1))
}

// AddVisitStateChanged (event add add_VisitStateChanged) dispatches through IGeovisitMonitor's vtable slot 9.
// The handler stays registered (and referenced by the runtime) until the
// returned token is passed to RemoveVisitStateChanged.
func (self *IGeovisitMonitor) AddVisitStateChanged(handler *TypedEventHandlerOfGeovisitMonitorAndGeovisitStateChangedEventArgs) (syswinrt.EventRegistrationToken, error) {
	result := new(syswinrt.EventRegistrationToken)
	r1, _, _ := syscall.SyscallN(self.LpVtbl[9], uintptr(unsafe.Pointer(self)), handler.Ptr(), uintptr(winrt.OutParam(unsafe.Pointer(result))))
	return *result, win32.ErrIfFailed(int32(r1))
}

// RemoveVisitStateChanged (event remove remove_VisitStateChanged) dispatches through IGeovisitMonitor's vtable slot 10,
// unregistering the VisitStateChanged handler the token was returned for.
func (self *IGeovisitMonitor) RemoveVisitStateChanged(token syswinrt.EventRegistrationToken) error {
	r1, _, _ := syscall.SyscallN(self.LpVtbl[10], uintptr(unsafe.Pointer(self)), uintptr(token.Value))
	return win32.ErrIfFailed(int32(r1))
}

// IGeovisitMonitorStatics is the WinRT interface Windows.Devices.Geolocation.IGeovisitMonitorStatics.
// IID: bcf976a7-bbf2-4cdd-95cf-554c82edfb87
// Exclusive to the Windows.Devices.Geolocation.GeovisitMonitor runtime class.
type IGeovisitMonitorStatics struct {
	syswinrt.IInspectable
}

// IID_IGeovisitMonitorStatics is the interface identifier for IGeovisitMonitorStatics.
var IID_IGeovisitMonitorStatics = win32.GUID{Data1: 0xbcf976a7, Data2: 0xbbf2, Data3: 0x4cdd, Data4: [8]byte{0x95, 0xcf, 0x55, 0x4c, 0x82, 0xed, 0xfb, 0x87}}

// GetLastReportAsync dispatches through IGeovisitMonitorStatics's vtable slot 6.
func (self *IGeovisitMonitorStatics) GetLastReportAsync() (*IAsyncOperationOfGeovisit, error) {
	result := new(*IAsyncOperationOfGeovisit)
	r1, _, _ := syscall.SyscallN(self.LpVtbl[6], uintptr(unsafe.Pointer(self)), uintptr(winrt.OutParam(unsafe.Pointer(result))))
	return *result, win32.ErrIfFailed(int32(r1))
}

// IGeovisitStateChangedEventArgs is the WinRT interface Windows.Devices.Geolocation.IGeovisitStateChangedEventArgs.
// IID: ceb4d1ff-8b53-4968-beed-4cecd029ce15
// Exclusive to the Windows.Devices.Geolocation.GeovisitStateChangedEventArgs runtime class.
type IGeovisitStateChangedEventArgs struct {
	syswinrt.IInspectable
}

// IID_IGeovisitStateChangedEventArgs is the interface identifier for IGeovisitStateChangedEventArgs.
var IID_IGeovisitStateChangedEventArgs = win32.GUID{Data1: 0xceb4d1ff, Data2: 0x8b53, Data3: 0x4968, Data4: [8]byte{0xbe, 0xed, 0x4c, 0xec, 0xd0, 0x29, 0xce, 0x15}}

// Visit (propget get_Visit) dispatches through IGeovisitStateChangedEventArgs's vtable slot 6.
func (self *IGeovisitStateChangedEventArgs) Visit() (*IGeovisit, error) {
	result := new(*IGeovisit)
	r1, _, _ := syscall.SyscallN(self.LpVtbl[6], uintptr(unsafe.Pointer(self)), uintptr(winrt.OutParam(unsafe.Pointer(result))))
	return *result, win32.ErrIfFailed(int32(r1))
}

// IGeovisitTriggerDetails is the WinRT interface Windows.Devices.Geolocation.IGeovisitTriggerDetails.
// IID: ea770d9e-d1c9-454b-99b7-b2f8cdd2482f
// Exclusive to the Windows.Devices.Geolocation.GeovisitTriggerDetails runtime class.
type IGeovisitTriggerDetails struct {
	syswinrt.IInspectable
}

// IID_IGeovisitTriggerDetails is the interface identifier for IGeovisitTriggerDetails.
var IID_IGeovisitTriggerDetails = win32.GUID{Data1: 0xea770d9e, Data2: 0xd1c9, Data3: 0x454b, Data4: [8]byte{0x99, 0xb7, 0xb2, 0xf8, 0xcd, 0xd2, 0x48, 0x2f}}

// ReadReports dispatches through IGeovisitTriggerDetails's vtable slot 6.
func (self *IGeovisitTriggerDetails) ReadReports() (*IVectorViewOfGeovisit, error) {
	result := new(*IVectorViewOfGeovisit)
	r1, _, _ := syscall.SyscallN(self.LpVtbl[6], uintptr(unsafe.Pointer(self)), uintptr(winrt.OutParam(unsafe.Pointer(result))))
	return *result, win32.ErrIfFailed(int32(r1))
}

// IPositionChangedEventArgs is the WinRT interface Windows.Devices.Geolocation.IPositionChangedEventArgs.
// IID: 37859ce5-9d1e-46c5-bf3b-6ad8cac1a093
// Exclusive to the Windows.Devices.Geolocation.PositionChangedEventArgs runtime class.
type IPositionChangedEventArgs struct {
	syswinrt.IInspectable
}

// IID_IPositionChangedEventArgs is the interface identifier for IPositionChangedEventArgs.
var IID_IPositionChangedEventArgs = win32.GUID{Data1: 0x37859ce5, Data2: 0x9d1e, Data3: 0x46c5, Data4: [8]byte{0xbf, 0x3b, 0x6a, 0xd8, 0xca, 0xc1, 0xa0, 0x93}}

// Position (propget get_Position) dispatches through IPositionChangedEventArgs's vtable slot 6.
func (self *IPositionChangedEventArgs) Position() (*IGeoposition, error) {
	result := new(*IGeoposition)
	r1, _, _ := syscall.SyscallN(self.LpVtbl[6], uintptr(unsafe.Pointer(self)), uintptr(winrt.OutParam(unsafe.Pointer(result))))
	return *result, win32.ErrIfFailed(int32(r1))
}

// IStatusChangedEventArgs is the WinRT interface Windows.Devices.Geolocation.IStatusChangedEventArgs.
// IID: 3453d2da-8c93-4111-a205-9aecfc9be5c0
// Exclusive to the Windows.Devices.Geolocation.StatusChangedEventArgs runtime class.
type IStatusChangedEventArgs struct {
	syswinrt.IInspectable
}

// IID_IStatusChangedEventArgs is the interface identifier for IStatusChangedEventArgs.
var IID_IStatusChangedEventArgs = win32.GUID{Data1: 0x3453d2da, Data2: 0x8c93, Data3: 0x4111, Data4: [8]byte{0xa2, 0x05, 0x9a, 0xec, 0xfc, 0x9b, 0xe5, 0xc0}}

// Status (propget get_Status) dispatches through IStatusChangedEventArgs's vtable slot 6.
func (self *IStatusChangedEventArgs) Status() (PositionStatus, error) {
	result := new(PositionStatus)
	r1, _, _ := syscall.SyscallN(self.LpVtbl[6], uintptr(unsafe.Pointer(self)), uintptr(winrt.OutParam(unsafe.Pointer(result))))
	return *result, win32.ErrIfFailed(int32(r1))
}

// IVenueData is the WinRT interface Windows.Devices.Geolocation.IVenueData.
// IID: 66f39187-60e3-4b2f-b527-4f53f1c3c677
// Exclusive to the Windows.Devices.Geolocation.VenueData runtime class.
type IVenueData struct {
	syswinrt.IInspectable
}

// IID_IVenueData is the interface identifier for IVenueData.
var IID_IVenueData = win32.GUID{Data1: 0x66f39187, Data2: 0x60e3, Data3: 0x4b2f, Data4: [8]byte{0xb5, 0x27, 0x4f, 0x53, 0xf1, 0xc3, 0xc6, 0x77}}

// Id (propget get_Id) dispatches through IVenueData's vtable slot 6.
func (self *IVenueData) Id() (string, error) {
	result := new(syswinrt.HSTRING)
	r1, _, _ := syscall.SyscallN(self.LpVtbl[6], uintptr(unsafe.Pointer(self)), uintptr(winrt.OutParam(unsafe.Pointer(result))))
	if err := win32.ErrIfFailed(int32(r1)); err != nil {
		return "", err
	}
	return winrt.TakeHString(*result), nil
}

// Level (propget get_Level) dispatches through IVenueData's vtable slot 7.
func (self *IVenueData) Level() (string, error) {
	result := new(syswinrt.HSTRING)
	r1, _, _ := syscall.SyscallN(self.LpVtbl[7], uintptr(unsafe.Pointer(self)), uintptr(winrt.OutParam(unsafe.Pointer(result))))
	if err := win32.ErrIfFailed(int32(r1)); err != nil {
		return "", err
	}
	return winrt.TakeHString(*result), nil
}
